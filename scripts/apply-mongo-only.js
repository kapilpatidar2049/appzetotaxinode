/**
 * Mongo-only: remove MySQL branches and query() imports. Run: node scripts/apply-mongo-only.js
 */
const fs = require("fs");
...
function stripCompoundNotMongo(src) {
  return src.replace(
    /if\s*\(\s*config\.dbClient\s*!==\s*["']mongo["']\s*\|\|\s*([^)]+)\)\s*return\s*;/g,
    "if (!$1) return;"
  );
}
...
function transformAll(s) {
  let prev = "";
  let out = s;
  let guard = 0;
  while (prev !== out && guard++ < 50) {
    prev = out;
    out = stripMongoElseBlocks(out);
    out = stripNotMongoBlocks(out);
    out = unwrapMongoOnlyIfs(out);
    out = stripCompoundNotMongo(out);
  }
  return out;
}
